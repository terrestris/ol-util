...
function _load_test_path_pattern_to_regexp() {
  return (_test_path_pattern_to_regexp = _interopRequireDefault(
    require('./test_path_pattern_to_regexp')
  ));
}
...
const globsToMatcher = globs => {
  ...
  const matchers = globs.map(each =>
    (_micromatch || _load_micromatch()).default.matcher(each, {dot: true})
  );
...
class SearchSource {
  ...
  _filterTestPathsWithStats(allPaths, testPathPattern) {
    ...
    if (testPathPattern) {
      const regex = (0,
      (_test_path_pattern_to_regexp || _load_test_path_pattern_to_regexp())
        .default)(testPathPattern);
