...
function _load_get_no_test_found_message() {
  return (_get_no_test_found_message = _interopRequireDefault(
    require('./get_no_test_found_message')
  ));
}
...
function _load_test_scheduler() {
  return (_test_scheduler = _interopRequireDefault(
...
function _load_test_sequencer() {
  return (_test_sequencer = _interopRequireDefault(
    require('./test_sequencer')
  ));
}
...
var _get_node_handles;

function _load_get_node_handles() {
  return (_get_node_handles = _interopRequireDefault(
    require('./get_node_handles')
  ));
}
...
const getTestPaths = (() => {
  var _ref = _asyncToGenerator(function*(
    ...
    const source = new (_search_source || _load_search_source()).default(
      context
    );
  ...
  var _ref3 = _asyncToGenerator(function*(_ref2) {
    ...
    if (globalConfig.listTests) {
      ...
      onComplete &&
        onComplete(
          (0,
          (_test_result_helpers || _load_test_result_helpers())
    ...
    if (!allTests.length) {
      ...
      (_get_no_test_found_message || _load_get_no_test_found_message())
        .default)(testRunData, globalConfig);
    ...
    if (globalConfig.detectOpenHandles) {
      collectHandles = (0,
      (_get_node_handles || _load_get_node_handles()).default)();
    }
    ...
    const results = yield new (
      _test_scheduler || _load_test_scheduler()
    ).default(
      globalConfig,
      {
        startRun: startRun
