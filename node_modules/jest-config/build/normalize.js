...
function _load_validate_pattern() {
  return (_validate_pattern = _interopRequireDefault(
    require('./validate_pattern')
  ));
}
...
function _load_get_max_workers() {
  return (_get_max_workers = _interopRequireDefault(
    require('./get_max_workers')
  ));
}
...
const normalizeReporters = (options, basedir) => {
  ...
  (0,
  (_reporter_validation_errors || _load_reporter_validation_errors())
...
const buildTestPathPattern = argv => {
  ...
  if (
    (0, (_validate_pattern || _load_validate_pattern()).default)(
      testPathPattern
    )
  ) {
...
function normalize(options, argv) {
  ...
  options = normalizePreprocessor(
    normalizeReporters(
      normalizeMissingOptions(
        normalizeRootDir(
          (0, (_set_from_argv || _load_set_from_argv()).default)(options, argv)
        )
      )
    )
  );
  ...
  (_get_max_workers || _load_get_max_workers()).default)(argv);
